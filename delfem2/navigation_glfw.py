...
#        self.camera = Camera(view_height)
    ...
    def motion(self, win_glfw, x, y, camera) -> None:
        (win_w, win_h) = glfw.get_window_size(win_glfw)
        ...
        if self.button == glfw.MOUSE_BUTTON_LEFT:
            if self.modifier == glfw.MOD_ALT:  # shift
                camera.rotation(self.mouse_x, self.mouse_y, self.mouse_pre_x, self.mouse_pre_y)
            if self.modifier == glfw.MOD_SHIFT:
                camera.translation(self.mouse_x, self.mouse_y, self.mouse_pre_x, self.mouse_pre_y)
