...
py::array_t<double> get_parameter_history_bvh(
  ...
  return py::array(
    py::buffer_info(
      bvh.frame_channel.data(), sizeof(double),
...
pyMeshTri3_RigBones_Octahedron(
  const dfm2::BioVisionHierarchy &bvh) {
  std::vector<double> vtx_xyz;
  std::vector<unsigned int> tri_vtx;
  dfm2::Mesh_RigBones_Octahedron(vtx_xyz, tri_vtx, bvh.bones);
  py::array_t<double> np_XYZ({(int) vtx_xyz.size() / 3, 3}, vtx_xyz.data());
  py::array_t<unsigned int> np_Tri({(int) tri_vtx.size() / 3, 3}, tri_vtx.data());
  return {np_XYZ, np_Tri};
}
...
void init_bvh(py::module &m) {
  ...
  m.def("get_meshtri3_rigbones_octahedron" , &pyMeshTri3_RigBones_Octahedron);
}